...
- **npm preview package**: 현재 npm `latest`는 `0.1.26` multi-platform release, repo package source는 `0.1.27` multi-platform 후보
...
- repo package source `0.1.27`은 native 실행 권한 보강을 포함한 multi-platform npm 배포 후보이며, npm `latest` 승격 전에는 optional native package publish와 smoke evidence가 필요합니다.
