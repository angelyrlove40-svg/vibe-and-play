...
공개 npm 패키지(`latest`는 2026-05-18 확인 기준 `0.1.27`):
...
2026-05-18 기준 npm 공개 배포는 완료된 상태입니다. 사용자 설치 안내는 `latest` 또는 `0.1.27+` 기준으로만 유지합니다.

- `vibemud@0.1.27`이 npm `latest`입니다.
- 6개 optional native package가 모두 `0.1.27`로 공개되어 있습니다: `@vibemud/native-darwin-arm64`, `@vibemud/native-darwin-x64`, `@vibemud/native-linux-arm64-gnu`, `@vibemud/native-linux-x64-gnu`, `@vibemud/native-win32-arm64-msvc`, `@vibemud/native-win32-x64-msvc`.
- clean install smoke에서 `npm install --prefix <tmp> vibemud@latest` 후 `vibemud`, `mudctl`, `vibemud-runtime`, `vibemud-hud`의 `--help` 실행을 확인했습니다.
- `0.1.26`은 npm publish는 성공했지만 일부 macOS 설치에서 native binary 실행 권한이 `0644`로 풀려 `EACCES`가 발생할 수 있어 `0.1.27`로 대체했습니다.
- `0.1.27`은 native/root postinstall에서 실행 권한을 보정하고, pack/test 검증에 회귀 방지를 추가했습니다.

## 현재 지원 상태

지원 표기는 실제 smoke 결과 기준입니다. **2026-05-18 현재 side-pane/HUD 자동화 검증 완료 대상은 tmux, cmux, macOS Ghostty뿐입니다. Codex와 iTerm2는 동작 실패가 확인되어 현재 미지원입니다.**
...
| npm package | ✅ 배포 완료 | npm `latest`는 2026-05-18 확인 기준 `vibemud@0.1.27`; darwin/linux/win32 x64·arm64 optional native packages publish 완료 |
...
- **npm package**: 현재 npm `latest`는 `0.1.27` multi-platform release
