...
현재 공개 `vibemud@0.1.25`는 **macOS Apple Silicon(darwin/arm64) host-only preview**였습니다. 이 package source는 다음 배포 후보(`0.1.26`)이며, multi-platform `latest`는 matching `@vibemud/native-*` optional package가 먼저 publish된 뒤에만 배포합니다.
